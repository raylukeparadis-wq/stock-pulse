...
export default function StockAnalyzer() {
  ...
  async function handleLookup(symbolOverride) {
    ...
    setChartExpanded(true); // a new lookup always re-opens the stock view
    startCooldown();
  ...
  function renderRecommendationCategories(categoryKeysToShow, showEmpty = false, collapsible = false) {
    ...
    return [
      ...
      const body = count === 0 ? (
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
          {items.map(item => {
            ...
            const isHeld = key === "long" && !!heldBySymbol[item.symbol]; // already in the paper portfolio
            ...
            return (
              <button
                ...
                title={
                  ...
                  isHeld ? "Already held in the paper portfolio — an active signal here means the position re-triggered, not necessarily a new buy" :
                  watchReason ? `Near miss: ${watchReason}` : undefined
                }
                style={{
                  background: isHeld ? `${itemColor}33` : `${itemColor}1a`,
                  border: isHeld ? `1px solid ${itemColor}` : `1px solid ${itemColor}66`,
              ...
              >
                ...
                {isHeld && <span style={{ fontSize: 9, marginLeft: 4, color: itemColor, opacity: 0.85, fontWeight: 600 }}>•held</span>}
      ...
      if (collapsible) {
        ...
        const heldCount = key === "long" ? items.filter(it => heldBySymbol[it.symbol]).length : 0;
        return (
          <div key={key} style={{ marginBottom: 10 }}>
            ...
            >
              ...
              {heldCount > 0 && <span style={{ fontSize: 11, color: "#64748b", fontWeight: 500 }}>· {heldCount} held</span>}
            </button>
  ...
  return (
    <div style={{
      ...
      <div style={{ maxWidth: 860, margin: "0 auto" }}>
        ...
        {quote && chartExpanded && (
          <>
            {/* Collapse control — lives inside the expanded view; collapsing hides the whole stock view (re-open by searching/selecting a stock) */}
            <button
            ...
            >
              ...
              <span style={{ fontSize: 11, color: "#475569", transform: "rotate(90deg)" }}>▶</span>
              <span style={{ fontSize: 11, color: "#475569", marginLeft: 4 }}>tap to hide</span>
            </button>
                              ...
                              return (
                                <div key={`${c.symbol}-${c.exitDate}-${i}`} style={{
                                  ...
                                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                    <span>
                                      <strong
                                        onClick={() => handleLookup(c.symbol)}
                                        style={{ color: "#3b82f6", cursor: (cooldown > 0 || loading) ? "not-allowed" : "pointer" }}
                                      >{c.symbol}</strong>
