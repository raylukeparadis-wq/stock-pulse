...
export default function StockAnalyzer() {
  ...
  async function handleLookup(symbolOverride) {
    ...
    try {
      ...
      // Uses default fetch mode (no explicit start/end) rather than pinning
      // to today's date: this naturally returns whatever the most recent
      // available 5-min bars are. If the market is open, that's today's
      // bars up to now. If closed, Alpaca simply has no bars for "today"
      // yet, so the most recent ones are automatically the last full
      // trading day instead -- no market-hours detection needed.
      // limit=100 covers a full ~6.5hr trading day (78 five-min bars) with
      // some buffer, without pulling in multiple days' worth.
      //
      // IMPORTANT: alpaca-proxy's rate-limit cooldown key is ip+route+symbol
      // -- it does NOT distinguish by timeframe. The daily bars request
      // above (timeframe=1Day) and this intraday request (timeframe=5Min)
      // both hit /bars for the same symbol, so firing this immediately
      // would very likely get silently 429'd by the cooldown from the
      // request that just completed. Delaying past the 1-second cooldown
      // window avoids that collision.
      setTimeout(() => {
        ...
        fetchBars(`${WORKER_URL}/bars?symbol=${t}&timeframe=5Min&start=${todayStr}&end=${todayStr}`)
          .then(bars => {
            ...
            // to the most recent available session instead. Small delay
            // to avoid colliding with step 1's own cooldown window.
            setTimeout(() => {
  ...
  return (
    <div style={{
      ...
      <div style={{ maxWidth: 860, margin: "0 auto" }}>
        ...
        <p style={{ fontSize: 9, color: "#334155", textAlign: "center", marginBottom: 8 }}>
          build: 2026-07-23-intraday-v3
        </p>
        <div style={{ textAlign: "center", marginBottom: 16 }}>
          <button
            onClick={() => {
              ...
              fetch(window.location.href + (window.location.href.includes('?') ? '&' : '?') + '_cb=' + Date.now(), { cache: "no-store" })
                .then(r => r.text())
                .then(html => {
                  const markers = [
                    { label: "Old text (pre-fix)", found: html.includes("No intraday data yet") },
                    { label: "Mid-fix text", found: html.includes("No intraday data available") },
                    { label: "Latest error-visible version", found: html.includes("Rate limited") },
                    { label: "Version marker (v3)", found: html.includes("intraday-v3") },
                  ];
                  setSourceCheckResult(markers);
                })
                .catch(err => setSourceCheckResult([{ label: "Fetch failed: " + err.message, found: false }]))
                .finally(() => setSourceCheckLoading(false));
            }}
            style={{
              background: "#1e293b", border: "1px solid #334155", borderRadius: 8,
              padding: "6px 14px", color: "#94a3b8", fontSize: 11, fontWeight: 600, cursor: "pointer",
            }}
          >
            {sourceCheckLoading ? "Checking..." : "Check what's actually deployed"}
          </button>
          {sourceCheckResult && (
            <div style={{ marginTop: 10, textAlign: "left", background: "#0f172a", border: "1px solid #1e293b", borderRadius: 8, padding: 12, fontSize: 11 }}>
              {sourceCheckResult.map((m, i) => (
                <div key={i} style={{ color: m.found ? "#22c55e" : "#475569", marginBottom: 4 }}>
                  {m.found ? "✓" : "✗"} {m.label}
                </div>
              ))}
            </div>
